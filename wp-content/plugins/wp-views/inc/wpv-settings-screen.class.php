...
class WPV_Settings_Screen {
    ...
    function init() {		
		...
		*/
		...
		// Bootstrap options
		add_filter( 'toolset_filter_toolset_register_settings_front-end-content_section',	array( $this, 'wpv_bootstrap_options' ), 30 );
	...
	*/
    ...
    function wpv_bootstrap_options( $sections ) {
        $is_disabled = '';
        $disabled_message = '';
			...
			<?php
				...
				foreach( $version_options as $option ) {
					printf(
						...
						$option['value'],
						checked( $option['value'] == $settings->wpv_bootstrap_version, true, false ),
		...
		<p>
			...
			echo sprintf( 
				...
				WPV_Admin_Messages::get_documentation_promotional_link( $documentation_link_args, 'https://wp-types.com/documentation/user-guides/view-layouts-101/' ),
	...
	*/
    ...
    function wpv_update_bootstrap_version_status() {
		...
		if ( null != $status ) {
			$settings->wpv_bootstrap_version = $status;
            $settings->save();
			wp_send_json_success();
		} else {
			wp_send_json_error();
