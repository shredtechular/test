...
if ( ! function_exists( 'avada_add_site_name_to_title' ) ) {
	...
	add_filter( 'wp_title', 'avada_add_site_name_to_title', 20, 1 );
}
...
if ( ! function_exists( 'avada_extract_shortcode_contents' ) ) {
	...
	function avada_extract_shortcode_contents( $m ) {
		...
		if ( in_array( $m[2], $shortcodes ) ) {
			...
			$content = preg_replace_callback( "/$pattern/s", 'avada_extract_shortcode_contents', rtrim( $m[5] ) . ' ' );
			return $content;
		}
...
<!--							<h1<?php echo $entry_title_class; ?>><?php echo $title; ?></h1>-->
						...
						<?//php if (is_page(925) || is_archive() ) : ?>
						<?php $shlink = "http://" . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"];
							if ( is_home() ) {
						  		$shtitle = get_the_title( get_option('page_for_posts', true) ); 
						  	} elseif ( is_archive() ) {
						  		$shtitle = single_cat_title( 'Stories Of A Jewish Community Day School; ', false );
						  	} else {
								$shtitle = get_the_title( $ID ); 
						  	}?>
						<?php echo do_shortcode('[sharing tagline="Share" tagline_color="#747474" title="' . $shtitle . '" link="' . $shlink . '" description="" pinterest_image="" icons_boxed="yes" icons_boxed_radius="45px" box_colors="#747474" icon_colors="#FFF" tooltip_placement="" backgroundcolor="" class="" id=""][/sharing]');?>
						<?//php endif;?>
					</div>

					<div class="fusion-one-fourth fusion-layout-column fusion-column-last fusion-spacing-yes fusion-page-title-secondary">
						...
						<?php if ( is_page(925) || is_archive() || is_single() || is_search() || is_home() ) : ?>
						<a class="sb-toggle-title-bar" href="#" style="vertical-align: middle;">View All Topics</a>
						<?php else : ?>
						&nbsp;	
						<?php endif;?>
					</div>
